...
CHorizontalBlurShader::CHorizontalBlurShader( )
{
	ZeroMemory( this, sizeof( CHorizontalBlurShader ) );
}

bool CHorizontalBlurShader::Initialize( ID3D11Device * device )
{
	HRESULT hr;
	ID3DBlob * VertexShaderBlob;
	if ( !CompileAndCreateVertexShader( device,
		L"HorizontalBlurVertexShader.hlsl", L"Shaders\\HorizontalBlurVertexShader.cso",
	...
	if ( !CompileAndCreatePixelShader( device,
		L"HorizontalBlurPixelShader.hlsl", L"Shaders\\HorizontalBlurPixelShader.cso",
		&m_PixelShader ) )
		return false;
...
void CHorizontalBlurShader::SetData( ID3D11DeviceContext * context, DirectX::FXMMATRIX& Projection,
	ID3D11ShaderResourceView * Texture, float Width )
{
	...
	( ( SMatrices* ) MappedResource.pData )->Width = Width;
...
void CHorizontalBlurShader::Render( ID3D11DeviceContext * context, UINT indexCount,
	DirectX::FXMMATRIX& Projection, ID3D11ShaderResourceView * Texture,
	float Width )
{
	SetData( context, Projection, Texture, Width );
	SetShaders( context );
	DrawIndexed( context, indexCount );
}

void CHorizontalBlurShader::SetShaders( ID3D11DeviceContext * context )
{
...
void CHorizontalBlurShader::Shutdown( )
{
