...
CAddTexturesShader::CAddTexturesShader( )
{
	ZeroMemory( this, sizeof( CAddTexturesShader ) );
}

bool CAddTexturesShader::Initialize( ID3D11Device * device )
{
	HRESULT hr;
	ID3DBlob * VertexShaderBlob;
	if ( !CompileAndCreateVertexShader( device,
		L"AddTexturesVertexShader.hlsl", L"Shaders\\AddTexturesVertexShader.cso",
	...
	if ( !CompileAndCreatePixelShader( device,
		L"AddTexturesPixelShader.hlsl", L"Shaders\\AddTexturesPixelShader.cso",
		&m_PixelShader ) )
		return false;
...
void CAddTexturesShader::SetData( ID3D11DeviceContext * context, DirectX::FXMMATRIX& Projection,
...
void CAddTexturesShader::Render( ID3D11DeviceContext * context, UINT indexCount,
	DirectX::FXMMATRIX& Projection, ID3D11ShaderResourceView * Texture, ID3D11ShaderResourceView * Texture2 )
{
	SetData( context, Projection, Texture, Texture2 );
	SetShaders( context );
	DrawIndexed( context, indexCount );
}

void CAddTexturesShader::SetShaders( ID3D11DeviceContext * context )
{
...
void CAddTexturesShader::Shutdown( )
{
